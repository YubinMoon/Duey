...
* Build types: `debug` (`com.terry.duey.debug`, app label `Duey Debug`), `stage` (`com.terry.duey.stage`, app label `Duey Stage`), `prod` (`com.terry.duey`, app label `Duey`)
* App versioning: edit `appVersionName` in `app/build.gradle.kts` using SemVer (`MAJOR.MINOR.PATCH`); `versionCode` is calculated from it.
...
* `app/src/main/java/com/terry/duey/model/TodoItem.kt`
  * `AppDate`, `TodoItem`, `Category` 정의.
  * `TodoItem`과 `Category`는 Room entity입니다.
* `app/src/main/java/com/terry/duey/data/AppDatabase.kt`
  * Room singleton database. 현재 DB 이름은 `todo_database`, version은 `1`, `exportSchema = false`.
* `app/src/main/java/com/terry/duey/data/TodoDao.kt`
  * 전체 일정 Flow 조회, 추가/수정/삭제, 다중 삭제, 완료 토글 쿼리.
* `app/src/main/java/com/terry/duey/data/CategoryDao.kt`
  * 카테고리 Flow 조회, 추가, 삭제, 이름 변경.
* `app/src/main/java/com/terry/duey/data/DateConverters.kt`
  * `AppDate` Room 저장을 위한 type converter.
* `app/src/main/java/com/terry/duey/viewmodel/TodoViewModel.kt`
  * `AndroidViewModel`.
...
* **Kotlin package:** Kotlin 소스 package와 경로가 `com.terry.duey`로 마이그레이션되었습니다. `namespace`와 소스 패키지가 일치합니다.
...
6. UI/Android 변경이 있고 기기가 준비되어 있으면 `.\gradlew.bat connectedStageAndroidTest`
