# Project: myTodo (Android / Jetpack Compose)

이 문서는 에이전트가 `myTodo` 프로젝트의 현재 상태를 빠르게 파악하고, 기존 구현 방식과 충돌하지 않게 작업하기 위한 기준입니다.
...
`myTodo`는 Jetpack Compose와 Material 3 기반의 할 일/일정 관리 Android 앱입니다. 하단 탭으로 숙제 목록, 달력 일정, 새 일정 추가, 더보기/관리 화면을 전환합니다. 현재 데이터는 Room DB에 저장되며, 카테고리 관리, 완료 상태, JSON 백업/복원, 홈 화면 Glance 위젯이 포함되어 있습니다.
...
2. `.\gradlew.bat test`
3. UI/Android 변경이 있으면 `.\gradlew.bat connectedAndroidTest`
