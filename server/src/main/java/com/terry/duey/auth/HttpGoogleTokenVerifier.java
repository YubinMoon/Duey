...
public class HttpGoogleTokenVerifier implements GoogleTokenVerifier {
    ...
    public HttpGoogleTokenVerifier(
            DueyProperties properties, RestClient.Builder restClientBuilder) {
    ...
    public Optional<GoogleUser> verify(String idToken) {
        if (properties.auth().googleClientId() == null
                || properties.auth().googleClientId().isBlank()) {
        ...
        if (response == null
                || !properties.auth().googleClientId().equals(response.path("aud").asString())) {
            return Optional.empty();
        }
        return Optional.of(
                new GoogleUser(
                        response.path("sub").asString(),
                        response.path("email").asString(),
                        response.path("name").asString(null)));
    }
}
